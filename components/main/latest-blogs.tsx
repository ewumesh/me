                    ...
                    <div className="grid gap-8 lg:grid-cols-2">
                        {latestBlogs.map((blog:any, index:number) => (
                            <article key={index} className="p-6 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
                                ...
                                <h2 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white"><a href={'/blog/' + blog._id}>{blog.title}</a></h2>
                                <p className="mb-5 font-light text-gray-500 dark:text-gray-400">{(blog?.content).slice(0,200)}</p>
                                <div className="flex justify-between items-center">
                                    <div className="flex items-center space-x-4">
                                        <img className="w-7 h-7 rounded-full" src={''} alt="Umesh Thapa" />
                                        <span className="font-medium dark:text-white">
                                            Umesh Thapa
                                        </span>
                                    </div>
                                    <a  href={'/blog/' + blog._id} className="inline-flex items-center font-medium text-primary-600 dark:text-white hover:underline">
                                        Read more
